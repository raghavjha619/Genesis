    ...
    const isPossibleMove = positionInList(position, gameState.possibleMoves);
      ...
      useGSAP(() => {
        if (pieceRef.current) {
          ...
          const boardRect =
            pieceRef.current.parentElement.getBoundingClientRect(); // Get board position
      ...
      }, [x, y]); // Dependencies: re-run when x or y changes
        ...
        {piece && (
          <div
            ...
            className={cn(
              ...
              isSelected &&
                "shadow-lg ring-2 ring-yellow-300 ring-offset-2 ring-offset-transparent"
            )}
            style={{
              position: "absolute",
              left: `${x}px`,
